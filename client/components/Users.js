...
export class Users extends React.Component {
  ...
  async handleDeleteUser(ev) {
    ...
    if (this.state.adminUsers.includes(ev.target.name)) {
      ...
      const updatedUsers = await this.state.users.filter(
        (u) => u !== ev.target.name
      )
  ...
  handleChecked(ev) {
    ...
    if (ev.target.checked) {
      ...
      const updatedAdminUsers = this.state.adminUsers.filter(
        (u) => u !== ev.target.name
      )
      this.setState({ adminUsers: updatedAdminUsers })
            ...
            {this.state.users.map((user) => {
              return (
                <tr key={++id}>
                  ...
                  <td>
                    <button type="button" onClick={this.handleResetPassword}>Reset</button>
                  </td>
                  <td>
                    <button type="button" name={user} value={id} onClick={this.handleDeleteUser}>
                      X
                    </button>
                  </td>
                  <td>
                    <input name={user} type="checkbox" onChange={this.handleChecked} />
                  </td>
                </tr>
              )
            })}
