...
export const addCartItem = (cartItem, isLoggedIn = false) => {
  return async (dispatch) => {
    ...
    if (isLoggedIn)
      newCartItem = (await axios.post('/api/cart/item', cartItem)).data
...
export const changeCartItem = (cartItem, isLoggedIn = false) => {
  return async (dispatch) => {
    ...
    if (isLoggedIn)
      updatedCartItem = (await axios.put('/api/cart/item', cartItem)).data
    //if not logged in, write to localStorage
...
export const removeCartItem = (cartItem, isLoggedIn = false) => {
  return async (dispatch) => {
    ...
    if (isLoggedIn)
      deleteSuccessful = (await axios.delete(`/api/cart/item/${cartItem.id}`))
        .status
    //if not logged in, write to localStorage
...
export default function cartReducer(state = [], action) {
  switch (action.type) {
    case ADD_CART_ITEM:
      return [
        ...state.filter((item) => item.artworkId !== action.cartItem.artworkId),
        action.cartItem,
      ]
